// pages/api/vault4/items.ts
import type { NextApiRequest, NextApiResponse } from 'next'
import { ObjectId } from 'mongodb'
import { getDb } from '../../../lib/db'

const COOKIE_NAME = 'vault4_token'
const COLLECTION = 'vaultitems4'

async function requireAuth(req: NextApiRequest): Promise<boolean> {
    const token = req.cookies?.[COOKIE_NAME]
    if (!token) return false
    const db = await getDb()
    const session = await db.collection('sessions4').findOne({ token })
    if (!session || !session.active) return false
    if (new Date() > new Date(session.expiresAt)) {
        await db.collection('sessions4').updateOne({ token }, { $set: { active: false } })
        return false
    }
    return true
}
