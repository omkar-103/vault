...
const SESSIONS_COLLECTION = 'sessions5'

async function createSession5(token: string): Promise<Date> {
...
export default async function handler(
    ...
    if (!isValid) {
        ...
        if (attempts >= 5) { // More lenient for vault 5? Or keep same. I'll keep same as others.
    ...
    const expiresAt = await createSession5(token)
